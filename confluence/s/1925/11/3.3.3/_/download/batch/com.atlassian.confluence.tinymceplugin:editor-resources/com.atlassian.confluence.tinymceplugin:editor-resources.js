...
(function(tinymce) {
	...
	tinymce.create('tinymce.AddOnManager', {
		...
		load : function(n, u, cb, s) {
            ...
            console.log("Script for [" + n + "] will not be loaded by TinyMCE. Please use Atlassian's requireResource.");
        }
	...
	tinymce.create('static tinymce.EditorManager', {
		...
		preInit : function() {
			...
			console.log("tinymce.documentBaseURL = " + tinymce.documentBaseURL);
...
// Atlassian - restore 3.0.0.1 modifier key handling otherwise cmd + key takes the place of ctrl + key on the Mac.
//						if (tinymce.isMac && o.ctrl != e.metaKey)
//							return;
//						else if (!tinymce.isMac && o.ctrl != e.ctrlKey)
//							return;
// Atlassian - 3.0.0.1 handling here:                        
...
tinymce.confluence.macrobrowser = (function($) { return {
    ...
    macroBrowserToolbarButtonClicked : function(options) {
        ...
        var settings = $.extend({
            ...
            mode : "insert",
            useEditorSelection: true
        }, options);

        var selectedHtml = settings.useEditorSelection && editor.selection.getContent();
...
(function($) {
    ...
    var imagetextKeys = ["align", "thumbnail", "border", "width", "height"],
         ...
         *
         * E.g. extracts "starryNight.png" from TST:Test Page^starryNight.png"
         */
    ...
    tinymce.create('tinymce.plugins.ConfluencePlugin', {
        init : function(ed, url) {
            ...
            ed.addCommand("mceConfUnlink", function (ui, val) {
                ...
                if (AJS.Editor.Adapter.isExternalLink($n.attr("href")) && $n.attr("aliasspecified") != "true") {
                    AJS.safe.post(AJS.params.contextPath + "/json/convertwikimarkuptoxhtmlwithoutpagewithspacekey.action",
                        {
            ...
            ed.addShortcut("ctrl+shift+i", ed.getLang("table.desc"), function() {
                return ed.execCommand("mceInsertTable", true); // need to pass in true to open the popup
            });

            ed.addShortcut("ctrl+shift+f", ed.getLang("fullscreen.desc"), "mceFullScreen");
            ...
            switch (n) {
                ...
                case 'confinsert':
                    ...
                    c.onRenderMenu.add(function(c, m) {
                        ...
                        AJS.$("#rte-featured-macros div").each(function() {
                            var item = AJS.$(this);
                        ...
                        m.onShowMenu.add(function() {
                            AJS.$(top.document).bind("keyup", closeMenuOnEsc);
                        ...
                        m.onHideMenu.add(function() {
                            ...
                            AJS.$(top.document).unbind("keyup", closeMenuOnEsc);
                        });
                    });
    ...
    tinymce.create('tinymce.plugins.PropertyPanel', {
        init : function(ed) {
            ...
            handler = function (ed, e, focusedEl) {
                ...
                if (!isRightClick(e) && !isPanelShowing()) {
                    AJS.trigger("user-focused-rte-element", data);
                }
            },
            ...
            AJS.bind("user-blurred-rte-element", function (e, data) {
                if (isPanelShowing()) {
                    if (!AJS.Confluence.PropertyPanel.current.isAnchor(data.focusedEl)) {
...
(function($) {
    ...
    AJS.bind("user-focused-rte-element", function (auiEvent, data) {
        var link = (data.focusedNodeName == "a") && data.focusedEl.href && data.focusedEl, $link;
        if (!link) {
            $link = $(data.focusedEl).closest("a[href]");   // might be a bold tag inside an a, for example
            link = $link[0];
        } else {
            $link = $(link);
        }
        ...
        var ppanel = AJS.Confluence.PropertyPanel.createFromButtonModel(link, buttons);
        ppanel.isAnchor = function (el) {
            return el == link || $.contains(link, el);
        };
...
(function($) {
    ...
    AJS.bind("user-focused-rte-element", function (auiEvent, data) {
        if (data.focusedNodeName != "img") return;

        var img = data.focusedEl,
            $img = $(img),
        ...
        AJS.Confluence.PropertyPanel.createFromButtonModel(img, buttons, { anchorIframe: AJS.Editor.Adapter.getEditorFrame() });
...
AJS.Editor.Adapter = (function($) { return {
    ...
    setStatusbarText: function(text) {
        var span = $("td.mceStatusbar span.hint");
        if (!span.length) {
    ...
    tinyMceOnInit : function() {
        ...
        this.setStatusbarText(this.getEditor().getLang(hint));
    ...
    /**
     * Replaces the container with the given text, which may be empty.
     ...
     * @param container  jQuery-wrapped element to replace
     ...
     */
    replaceWithTextAndGetRange: function(element, text, collapse) {
        text = text || "";
    ...
    initTinyMce: function() {
        ...
        t.settings = {
            ...
            plugins: "table,paste,emotions,fullscreen,confluence,contextmenu,autocomplete,safari,propertypanel",
